...
// ============================================================
// The core reset function. Pure: takes state, returns new state.
// If today !== lastResetDate, flip all completed/failed
// daily/custom quests back to active with fresh dueDate.
// ============================================================
function applyDailyResetIfNeeded(prev: GameState): GameState {
  ...
  if (count > 0) {
    newSystemLog.unshift({ id: generateId(), type: 'system', message: '[DAILY RESET] ' + count + ' quest' + (count > 1 ? 's' : '') + ' reactivated. New day, new hunt.', timestamp: new Date().toISOString() });
  }
...
export function GameProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<GameState>(() => {
    // Apply reset immediately on first load
    return applyDailyResetIfNeeded(loadState());
  });
  ...
  // ============================================================
  // POLL EVERY SECOND: check if the date has changed.
  // This catches midnight transitions even if the app is idle.
  // If the date changed, apply the reset.
  // ============================================================
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setState(prev => {
        if (prev.lastResetDate === today) return prev;
        console.log('[Game] Day changed! Running daily reset.');
        return applyDailyResetIfNeeded(prev);
      });
    }, 1000);
    return () => clearInterval(interval);
  }, []);

  // Also handle visibility change (user switches back to tab/app)
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') {
        setState(prev => applyDailyResetIfNeeded(prev));
      }
    };
  ...
  // Check expired quests every minute
    ...
    setState(prev => {
      ...
      const newQuests = prev.quests.map(q => {
        ...
        if (now > due) {
          ...
          if (level < oldLevel) newSystemLog.unshift({ id: generateId(), type: 'xpLoss', message: 'LEVEL DOWN! Dropped to Level ' + level + '.', timestamp: new Date().toISOString() });
          if (rankChanged) newSystemLog.unshift({ id: generateId(), type: 'rankDown', message: 'RANK DOWN! Now ' + RANK_NAMES[newRank] + ' (' + newRank + '-Rank).', timestamp: new Date().toISOString() });
