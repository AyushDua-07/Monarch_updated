...
function createDefaultUser(): User {
  return {
    ...
    punishmentsEnabled: true, createdAt: new Date().toISOString(), lastActiveDate: getLocalDateStr(),
  };
}
...
function loadState(): GameState {
  try {
    ...
    if (saved) {
      ...
      const systemLog = Array.isArray(p.systemLog) ? p.systemLog : [{ id: generateId(), type: 'system', message: 'System migrated.', timestamp: new Date().toISOString() }];
...
function applyDailyResetIfNeeded(prev: GameState): GameState {
  const today = getLocalDateStr();
  if (prev.lastResetDate === today) return prev;
  const newDueDate = getEndOfToday().toISOString();
  ...
  const newQuests = prev.quests.map(q => {
    ...
    delete reset.lockedUntil; delete reset.failedAt;
...
export function GameProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<GameState>(() => applyDailyResetIfNeeded(loadState()));
  useEffect(() => { saveState(state); }, [state]);

  useEffect(() => {
    const interval = setInterval(() => {
      const today = getLocalDateStr();
      setState(prev => { if (prev.lastResetDate === today) return prev; return applyDailyResetIfNeeded(prev); });
    }, 1000);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    const handle = () => { if (document.visibilityState === 'visible') setState(prev => applyDailyResetIfNeeded(prev)); };
    ...
    setState(prev => {
      ...
      const newQuests = prev.quests.map(q => {
        ...
        if (now > due) {
          ...
          newUser.level = level; newUser.currentXP = currentXP;
          ...
          if (level < oldLevel) newSystemLog.unshift({ id: generateId(), type: 'xpLoss', message: 'LEVEL DOWN! Level ' + level + '.', timestamp: new Date().toISOString() });
          if (rankChanged) newSystemLog.unshift({ id: generateId(), type: 'rankDown', message: 'RANK DOWN! ' + RANK_NAMES[newRank] + ' (' + newRank + ').', timestamp: new Date().toISOString() });
          // Log the failure so HuntMap can track it historically
          newLogs.unshift({ id: generateId(), type: q.targetType, title: 'Failed: ' + q.title, notes: 'auto-failed', difficulty: q.demonLevel, xpEarned: 0, statImpacts: {}, questId: q.id, questTitle: q.title, createdAt: new Date().toISOString() } as ActivityLog);
      ...
      return { ...prev, user: newUser, quests: newQuests, logs: newLogs, systemLog: newSystemLog.slice(0, 200) };
    });
  }, []);

  useEffect(() => {
    const t = setTimeout(() => checkExpiredQuests(), 500);
    const i = setInterval(() => checkExpiredQuests(), 60000);
  ...
  const completeQuest = useCallback((questId: string, notes?: string) => {
    ...
    setState(prev => {
      ...
      newUser.totalXP += xpEarned; newUser.currentXP += xpEarned;
      ...
      if (rankChanged) newSystemLog.unshift({ id: generateId(), type: 'rankUp', message: 'RANK UP! ' + RANK_NAMES[newRank] + ' (' + newRank + ').', timestamp: new Date().toISOString() });
  ...
  const failQuest = useCallback((questId: string) => {
    setState(prev => {
      ...
      // Log the abandonment so HuntMap tracks it
      const failLog = { id: generateId(), type: quest.targetType, title: 'Abandoned: ' + quest.title, notes: 'abandoned', difficulty: quest.demonLevel, xpEarned: 0, statImpacts: {}, questId: quest.id, questTitle: quest.title, createdAt: new Date().toISOString() } as ActivityLog;
      return { ...prev, user: newUser, logs: [failLog, ...prev.logs],
  ...
  const addQuest = useCallback((quest: Omit<Quest, 'id' | 'createdAt'>) => {
    ...
    setState(prev => ({ ...prev, quests: [nq, ...prev.quests],
      systemLog: [{ id: generateId(), type: 'questCreated' as const, message: 'NEW QUEST: "' + quest.title + '" Demon: ' + DEMON_LEVELS[quest.demonLevel].name + '. +' + quest.xpReward + '/-' + quest.xpPenalty + ' XP.', timestamp: new Date().toISOString() }, ...prev.systemLog].slice(0, 200) }));
  }, []);

  const deleteQuest = useCallback((questId: string) => { setState(prev => ({ ...prev, quests: prev.quests.filter(q => q.id !== questId) })); }, []);
  const editQuest = useCallback((questId: string, updates: Partial<Omit<Quest, 'id' | 'status' | 'createdAt'>>) => { setState(prev => ({ ...prev, quests: prev.quests.map(q => q.id === questId ? { ...q, ...updates } : q) })); }, []);
  const allocateStat = useCallback((stat: keyof UserStats) => { setState(prev => { if (prev.user.statPoints <= 0) return prev; return { ...prev, user: { ...prev.user, statPoints: prev.user.statPoints - 1, stats: { ...prev.user.stats, [stat]: prev.user.stats[stat] + 1 } } }; }); }, []);
