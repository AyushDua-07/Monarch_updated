...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const unlockQuestsAtMidnight = useCallback(() => {
    setState(prev => {
      ...
      const lockedQuests = prev.quests.filter(q => q.status === 'completed' && q.lockedUntil);
  ...
  const validateLockedQuests = useCallback(() => {
    setState(prev => {
      ...
      const oldLockedQuests = prev.quests.filter(q => {
        if (q.status !== 'completed' || !q.lockedUntil) return false;
        const lockedUntilTime = new Date(q.lockedUntil);
    ...
    setState(prev => {
      ...
      return {
        ...
        quests: prev.quests.map(q =>
          q.id === questId
            ? { ...q, status: 'locked' as QuestStatus, currentProgress: q.targetValue, completedAt: new Date().toISOString() }
            : q
        ),
        systemLog: newSystemLog.slice(0, 200),
      };
