...
id: generateId(),
type: ‘system’,
message: ‘System migrated.’,
timestamp: new Date().toISOString(),
}];
}
const lastResetDate = parsed.lastResetDate || ‘’;
return { user, quests, logs, systemLog, lastResetDate };
}
} catch (e) {
...
// ═══════════════════════════════════════════════════════════
// DAILY RESET — runs once per new day
//
// Simple: if today is a new day, every completed/failed
// daily or custom quest goes back to active.
// The UI (Quests.tsx) handles hiding custom quests on
// their off-days via isQuestForToday().
// ═══════════════════════════════════════════════════════════
...
```
  console.log('[Game] Daily reset:', prev.lastResetDate, '→', today);
  ...
  const newQuests = prev.quests.map(q => {
    // Only reset completed or failed quests
    if (q.status !== 'completed' && q.status !== 'failed') return q;
    // Only reset daily and custom frequency (not weekly)
    ...
    delete (reset as any).lockedUntil;
    delete (reset as any).failedAt;
    return reset;
  });
  ...
  if (count > 0) {
    newSystemLog.unshift({
      id: generateId(),
      type: 'system',
      message: `⏰ [DAILY RESET] ${count} quest${count > 1 ? 's' : ''} reactivated. The hunt continues.`,
      timestamp: new Date().toISOString(),
    });
  }
...
// ═══════════════════════════════════════════════════════════
// CHECK EXPIRED — auto-fail active quests past their dueDate
// Skips custom quests not scheduled for today
// ═══════════════════════════════════════════════════════════
...
```
  const newQuests = prev.quests.map(q => {
    ...
    // Don't auto-fail custom quests on off-days
    ...
    if (now > due) {
      ...
      const failed = { ...q, status: 'failed' as QuestStatus };
      (failed as any).failedAt = new Date().toISOString();
      return failed;
    }
    return q;
  });
...
// ═══════════════════════════════════════════════════════════
// STARTUP + MIDNIGHT TIMER
// ═══════════════════════════════════════════════════════════
useEffect(() => {
runDailyReset();
const startupTimer = setTimeout(() => checkExpiredQuests(), 200);
...
const expireInterval = setInterval(() => checkExpiredQuests(), 60000);
