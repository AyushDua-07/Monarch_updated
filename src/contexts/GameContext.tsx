...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    setState(prev => {
      ...
      newSystemLog.unshift({ id: generateId(), type: 'system', message: `DAILY RESET: ${resetCount} quest${resetCount > 1 ? 's' : ''} renewed.`, timestamp: new Date().toISOString() });
      return { ...prev, quests: newQuests, systemLog: newSystemLog.slice(0, 200) };
    });
  }, []);
  ...
  const failQuest = useCallback((questId: string) => {
    setState(prev => {
      ...
      const sl = [...prev.systemLog];
      sl.unshift({ id: generateId(), type: 'questFailed', message: `QUEST ABANDONED: "${quest.title}" — ${penalty} XP penalty.`, xpChange: -penalty, timestamp: new Date().toISOString() });
      return { ...prev, user: u, quests: prev.quests.map(q => q.id === questId ? { ...q, status: 'failed' as QuestStatus } : q), systemLog: sl.slice(0, 200) };
    });
  }, []);
  ...
  return (
    <GameContext.Provider value={{ ...state, completeQuest, failQuest, addQuest, deleteQuest, allocateStat, updateUserName, togglePunishments, setActiveTitle, clearAllData }}>
      {children}
