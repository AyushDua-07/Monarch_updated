...
/** Check if a quest is relevant for today */
...
// Day-aware: only count today's quests that are scheduled for today
const todayCompleted = quests.filter(q =>
  q.status === 'completed' && q.completedAt?.startsWith(todayStr) && isQuestForToday(q)
).length;
const todayActive = quests.filter(q =>
  q.status === 'active' && isQuestForToday(q)
).length;
