...
function getCellRing(count: number): string {
  if (count >= 4) return 'ring-1 ring-emerald-400/30';
  return '';
}
...
export default function HuntMap() {
  ...
  const questsByDay = useMemo(() => {
    ...
    quests.forEach(q => {
      if (q.completedAt) { const d = q.completedAt.split('T')[0]; if (!m[d]) m[d] = { completed: 0, failed: 0 }; m[d].completed += 1; }
      if ((q as any).failedAt) { const d = ((q as any).failedAt as string).split('T')[0]; if (!m[d]) m[d] = { completed: 0, failed: 0 }; m[d].failed += 1; }
    });
    return m;
  }, [quests]);
    ...
    for (let w = 0; w < numWeeks; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (dayNum < 1 || dayNum > daysInMonth) {
          ...
          let total = comp + fail;
          if (ds === todayStr) total += quests.filter(q => q.status === 'active' && isQuestForToday(q)).length;
          week.push({ date: ds, day: dayNum, xp: dailyXP[ds] || 0, completed: comp, failed: fail, total: total, isToday: ds === todayStr, isEmpty: false });
        }
      }
      weeks.push(week);
    }
  ...
  const last7 = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) { const d = new Date(now); d.setDate(now.getDate() - i); const ds = d.toISOString().split('T')[0]; days.push({ label: d.toLocaleString('default', { weekday: 'narrow' }), xp: dailyXP[ds] || 0 }); }
    return days;
  }, [dailyXP, now]);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-4">
          <button onClick={prevMonth} className="w-8 h-8 flex items-center justify-center rounded-sm bg-white/[0.03] text-gray-400 hover:text-cyan-400 hover:bg-white/[0.06] transition-all">
            <ChevronLeft size={16} />
          </button>
          ...
          <button onClick={nextMonth} className={'w-8 h-8 flex items-center justify-center rounded-sm transition-all ' + (isCurrentMonth ? 'bg-white/[0.02] text-gray-700 cursor-default' : 'bg-white/[0.03] text-gray-400 hover:text-cyan-400 hover:bg-white/[0.06]')} disabled={isCurrentMonth}>
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {DAY_HEADERS.map((d, i) => (
              <div key={i} className="text-center py-1">
                <span className="text-[9px] text-gray-500 font-mono font-medium">{d}</span>
              </div>
            ))}
          </div>
            ...
            {monthGrid.map((week, wi) => (
              <div key={wi} className="grid grid-cols-7 gap-1">
                {week.map((cell, di) => {
                  ...
                  return (
                    <div key={di}
                      onClick={() => { if (!cell.isEmpty) setSelectedDay(isSelected ? null : cell); }}
                      className={
                        'relative flex items-center justify-center rounded-sm transition-all ' +
                        (cell.isEmpty ? 'bg-transparent' : getCellBg(cell.completed) + ' ' + getCellRing(cell.completed)) + ' ' +
                        (cell.isToday && !cell.isEmpty ? 'ring-1 ring-cyan-400/70 ring-offset-1 ring-offset-[#0a0e1e]' : '') + ' ' +
                        (isSelected && !cell.isEmpty ? 'ring-1 ring-white/40 ring-offset-1 ring-offset-[#0a0e1e]' : '') + ' ' +
                    ...
                    >
                      {!cell.isEmpty && (
                        <>
                          <span className={'text-[10px] font-mono leading-none ' + (cell.isToday ? 'text-cyan-400 font-bold' : cell.completed > 0 ? 'text-white/90' : 'text-gray-600')}>{cell.day}</span>
                          {cell.completed > 0 && (
                            <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 flex gap-px">
                              ...
                              {cell.completed > 4 && <div className="w-1 h-1 rounded-full bg-emerald-300" />}
                            </div>
                          )}
                        </>
                      )}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex gap-0.5">
              {[0, 1, 2, 3, 5].map((v, i) => (
                <div key={i} className={'w-2.5 h-2.5 rounded-sm ' + getCellBg(v)} />
              ))}
            </div>
            ...
            {(selectedDay.completed > 0 || selectedDay.failed > 0 || selectedDay.xp > 0) ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 bg-emerald-500/5 border border-emerald-500/10 rounded-sm text-center">
                    <p className="font-mono text-lg font-bold text-emerald-400 leading-none">{selectedDay.completed}</p>
                    <p className="text-[8px] text-gray-500 font-mono mt-1 uppercase">Completed</p>
                  </div>
                  <div className="p-2 bg-red-500/5 border border-red-500/10 rounded-sm text-center">
                    <p className="font-mono text-lg font-bold text-red-400 leading-none">{selectedDay.failed}</p>
                    <p className="text-[8px] text-gray-500 font-mono mt-1 uppercase">Failed</p>
                  </div>
                  <div className="p-2 bg-cyan-500/5 border border-cyan-500/10 rounded-sm text-center">
                    <p className="font-mono text-lg font-bold text-cyan-400 leading-none">{selectedDay.total}</p>
                    <p className="text-[8px] text-gray-500 font-mono mt-1 uppercase">Total</p>
                  </div>
                </div>
                ...
                {selectedDay.total > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[9px] text-gray-400 font-mono">Completion Rate</span>
                      <span className="text-[9px] text-white font-mono font-medium">{Math.round((selectedDay.completed / selectedDay.total) * 100)}%</span>
                    </div>
                ...
                {selectedDay.xp > 0 && (
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <span className="text-xs text-cyan-400 font-mono font-bold">+{selectedDay.xp} XP earned</span>
                  </div>
                )}
              </div>
            ) : (
              <div className="text-center py-2">
                <p className="text-[10px] text-gray-500 font-mono">{selectedDay.isToday ? 'No activity yet - start a quest!' : 'Rest day'}</p>
              </div>
            )}
          </div>
        )}
      ...
      {/* Empty state */}
      {!hasAnyData && (
        <div className="text-center py-4 px-4 bg-white/[0.01] border border-dashed border-white/[0.06] rounded-sm">
