...
// LOCAL date string - device timezone, not UTC
...
export default function HuntMap() {
  ...
  // XP per day from logs - using LOCAL dates
  const dailyXP = useMemo(() => {
    const m: Record<string, number> = {};
    logs.forEach(l => {
      const d = toLocalDateStr(new Date(l.createdAt));
      m[d] = (m[d] || 0) + l.xpEarned;
    });
    return m;
  }, [logs]);

  // Quest completions/failures per day - using LOCAL dates
  const questsByDay = useMemo(() => {
    ...
    quests.forEach(q => {
      ...
      if ((q as any).failedAt) {
        const d = toLocalDateStr(new Date((q as any).failedAt as string));
        if (!m[d]) m[d] = { completed: 0, failed: 0 };
        m[d].failed += 1;
      }
    });
  ...
  const monthGrid = useMemo(() => {
    ...
    return weeks;
  }, [viewMonth, viewYear, dailyXP, questsByDay, quests, todayStr]);
  ...
  const todayStats = useMemo(() => {
    const todayCompleted = quests.filter(q => q.status === 'completed' && q.completedAt && toLocalDateStr(new Date(q.completedAt)) === todayStr && isQuestForToday(q)).length;
  ...
  }, [quests, dailyXP, todayStr]);

  const weekStats = useMemo(() => {
    const weekStart = new Date(now); weekStart.setDate(now.getDate() - now.getDay()); weekStart.setHours(0, 0, 0, 0);
    let weekXP = 0, weekQ = 0;
    Object.entries(dailyXP).forEach(([d, v]) => {
      const parts = d.split('-');
      const dt = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
      if (dt >= weekStart) { weekXP += v; weekQ++; }
    });
    ...
    Object.entries(dailyXP).forEach(([d, v]) => {
      const parts = d.split('-');
      const dt = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
      if (dt >= lastWeekStart && dt < weekStart) lastWeekXP += v;
    });
      ...
      <div>
        ...
        <div className="w-full">
          <div className="grid grid-cols-7 gap-1 mb-1">
            {DAY_HEADERS.map((d, i) => (
              <div key={i} className="text-center py-1"><span className="text-[9px] text-gray-500 font-mono font-medium">{d}</span></div>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex gap-0.5">{[0, 1, 2, 3, 5].map((v, i) => (<div key={i} className={'w-2.5 h-2.5 rounded-sm ' + getCellBg(v)} />))}</div>
              ...
              <div className="space-y-3">
                ...
                {selectedDay.total > 0 && (
                  <div>
                    ...
                    <div className="h-2 bg-white/[0.04] rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-700 ease-out" style={{ width: (selectedDay.total > 0 ? (selectedDay.completed / selectedDay.total) * 100 : 0) + '%', backgroundColor: selectedDay.completed === selectedDay.total ? '#10b981' : '#22c55e' }} /></div>
                  </div>
                )}
                {selectedDay.xp > 0 && (
                  <div className="flex items-center gap-2 pt-1"><Zap size={12} className="text-cyan-400" /><span className="text-xs text-cyan-400 font-mono font-bold">+{selectedDay.xp} XP earned</span></div>
                )}
              </div>
            ) : (
