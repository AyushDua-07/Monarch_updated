...
function getCellBg(completed: number, failed: number): string {
  ...
  if (total === 0) return 'bg-white/[0.03]';
  if (completed === 0 && failed > 0) return 'bg-red-900/40';
  if (completed === 1) return 'bg-emerald-900/60';
  if (completed === 2) return 'bg-emerald-700/60';
  if (completed <= 4) return 'bg-emerald-500/70';
  return 'bg-emerald-400/80';
}

function getCellRing(completed: number): string {
  if (completed >= 4) return 'ring-1 ring-emerald-400/30';
  return '';
}
...
export default function HuntMap() {
  ...
  // Build history from LOGS (permanent, never wiped by daily reset)
  // Completed logs have xpEarned > 0 and title starts with "Quest: "
  // Failed/abandoned logs have xpEarned === 0 and title starts with "Failed: " or "Abandoned: "
  const dailyHistory = useMemo(() => {
    ...
    logs.forEach(l => {
      ...
      if (l.xpEarned > 0) {
        m[d].completed += 1;
      } else if (l.title.startsWith('Failed:') || l.title.startsWith('Abandoned:') || l.notes === 'abandoned' || l.notes === 'auto-failed') {
        m[d].failed += 1;
      }
    });
    return m;
    ...
    for (let w = 0; w < numWeeks; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (dayNum < 1 || dayNum > daysInMonth) {
          ...
          let total = comp + fail;
          // For today, also count active quests
          if (ds === todayStr) {
  ...
  // Month summary from historical logs
  ...
  const todayStats = useMemo(() => {
    ...
    return { xp: hist?.xp || 0, completed: hist?.completed || 0, active: todayActive };
  ...
  const weekStats = useMemo(() => {
    ...
    Object.entries(dailyHistory).forEach(([d, v]) => {
      ...
      if (dt >= weekStart) { weekXP += v.xp; if (v.completed > 0) weekDays++; }
    });
  ...
  // Last 7 days bar
  const last7 = useMemo(() => {
    const days: { label: string; xp: number; completed: number }[] = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(now); d.setDate(now.getDate() - i);
      const ds = toLocalDateStr(d); const hist = dailyHistory[ds];
      days.push({ label: d.toLocaleString('default', { weekday: 'narrow' }), xp: hist?.xp || 0, completed: hist?.completed || 0 });
    }
    return days;
  }, [dailyHistory, now]);
      ...
      <div>
                  ...
                  return (
                    <div key={di}
                      ...
                      className={
                        ...
                        (cell.isEmpty ? 'bg-transparent' : getCellBg(cell.completed, cell.failed) + ' ' + getCellRing(cell.completed)) + ' ' +
                    ...
                    >
                      {!cell.isEmpty && (
                        <>
                          <span className={'text-[10px] font-mono leading-none ' + (cell.isToday ? 'text-cyan-400 font-bold' : cell.completed > 0 ? 'text-white/90' : cell.failed > 0 ? 'text-red-400/70' : 'text-gray-600')}>{cell.day}</span>
                          {cell.completed > 0 && (
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex gap-0.5">{[0, 1, 2, 3, 5].map((v, i) => (<div key={i} className={'w-2.5 h-2.5 rounded-sm ' + getCellBg(v, 0)} />))}</div>
