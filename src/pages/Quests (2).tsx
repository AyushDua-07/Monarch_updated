...
export default function Quests() {
  ...
  const activeQuests = quests.filter(q => {
    ...
    // Custom frequency quests only show on their selected days
    if (q.frequency === 'custom' && q.selectedDays && q.selectedDays.length > 0) {
      return q.selectedDays.includes(todayDayIndex);
    }
    return true;
  });
              ...
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-600 font-mono">
                  {tab === 'active'
                    ...
                    : tab === 'completed'
                    ? 'No completed quests yet. Finish quests to see them locked here.'
...
function QuestCard({ quest, index, onComplete, onFail, onDelete, onEdit }: {
  ...
  // Lock color: emerald for completed, red for failed
  const lockColor = isCompleted ? 'amber' : 'red';
      ...
      <SystemCard className={`${isLocked ? `opacity-60 ${lockBgClass}` : ''} ${isUrgent ? 'border-red-500/30' : ''}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Time remaining — only for active quests */}
            {!isLocked && (
            ...
            {/* Action Buttons — active quests only */}
            {!isLocked && (
              <div className="flex items-center gap-2 mt-2">
                <motion.button
                  whileTap={{ scale: 0.9 }}
            ...
            {/* Locked state actions — both completed and failed */}
            {isLocked && (
              <div className="flex items-center gap-2 mt-2">
                <button
                  onClick={onDelete}
