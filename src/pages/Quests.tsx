          ...
          >
            ...
            >
              ...
              <div className="p-5 space-y-5">
                ...
                {/* Activity Type & Demon Level */}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase tracking-wider font-mono block mb-1.5">Activity Type</label>
                    <select
                      value={qTargetType}
                  ...
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase tracking-wider font-mono block mb-1.5">Demon Level</label>
                    <select
                      value={qDemonLevel}
                      onChange={e => setQDemonLevel(Number(e.target.value) as DemonLevel)}
                      className="w-full bg-white/[0.05] border border-white/10 rounded-sm px-3 py-2.5 text-sm text-white focus:border-cyan-500/50 focus:outline-none"
                    >
                      {Object.entries(DEMON_LEVELS).map(([key, config]) => (
                        <option key={key} value={key} className="bg-gray-900">
                          {config.name}
                        </option>
                      ))}
        ...
        <div className="flex items-start gap-3">
          ...
          >
            <img
              ...
              className={`w-12 h-12 object-contain ${isLocked ? 'grayscale' : ''} ${isFailed ? 'grayscale opacity-50' : ''}`}
              style={{
                filter: !isLocked && !isFailed ? `drop-shadow(0 0 4px ${demonConfig.color}60)` : undefined,
              }}
            />
            {isLocked && (
