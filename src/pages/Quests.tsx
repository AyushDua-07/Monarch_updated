/*
 * Quests Page — Quest management with day-aware filtering
 * Custom-day quests only appear on their scheduled days across ALL tabs
 * Active, Completed (locked), Failed (locked) — all reactivate at midnight
 */
...
/** Check if a quest is relevant for today (daily/weekly always, custom only on selected days) */
...
export default function Quests() {
  ...
  // All tabs are day-aware: custom quests only show on their scheduled days
            ...
            <SystemCard>
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-600 font-mono">
                  {tab === 'active'
                    ? 'No active quests for today. Create one to begin your journey.'
                    : tab === 'completed'
                    ? 'No completed quests for today yet.'
                    : 'No failed quests for today. Keep up the good work!'}
                </p>
              </div>
            </SystemCard>
          )}
...
function QuestCard({ quest, index, onComplete, onFail, onDelete, onEdit }: {
  ...
  const lockTextClass = isCompleted ? 'text-amber-400' : 'text-red-400';
  const lockBgClass = isCompleted ? 'border-amber-500/20' : 'border-red-500/20';
  const lockBadgeBg = isCompleted ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20';

  // Show scheduled days for custom quests
  const dayLabels = quest.frequency === 'custom' && quest.selectedDays
    ? quest.selectedDays.map(d => DAY_NAMES[d]).join(', ')
    : null;
  ...
  return (
    ...
    >
      <SystemCard className={`${isLocked ? `opacity-60 ${lockBgClass}` : ''} ${isUrgent ? 'border-red-500/30' : ''}`}>
        <div className="flex items-start gap-3">
          <div
            className="w-14 h-14 rounded-md flex items-center justify-center shrink-0 overflow-hidden relative"
            style={{
          ...
          >
            <img
              ...
              style={{
                filter: !isLocked ? `drop-shadow(0 0 4px ${demonConfig.color}60)` : 'grayscale(1)',
              }}
            />
            {isLocked && (
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center rounded-md">
                <Lock size={16} className={lockTextClass} />
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isCompleted && (
                <span className={`text-[8px] px-1.5 py-0.5 rounded-sm font-mono ${lockBadgeBg}`}>
                  🔒 Completed
                </span>
              )}
              {isFailed && (
            ...
            {isFailed && (quest as any).failedAt && (
              <p className="text-[10px] text-red-400/60 font-mono mt-1">
                ✗ Abandoned {new Date((quest as any).failedAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </p>
            )}
            {isFailed && !(quest as any).failedAt && (
              <p className="text-[10px] text-red-400/60 font-mono mt-1">
                ✗ Abandoned — {quest.xpPenalty} XP penalty applied
              </p>
            )}
            ...
            {isLocked && (
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className={`text-[9px] ${isCompleted ? 'text-emerald-400/60' : 'text-red-400/60'} ml-auto font-mono`}>
